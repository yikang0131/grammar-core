...
from transformers import Qwen2ForCausalLM, GPTNeoXForCausalLM, Qwen3ForCausalLM
...
class IntervenableGPTNeoXForCausalLM(GPTNeoXForCausalLM):
    ...
    def intervenable_forward(
        ...
        **intervention_kwargs,
        ...
        intervention_kwargs:
        {
            "embed_in": torch.FloatTensor,
            "layers[0]": torch.FloatTensor,
            "layers[1]": torch.FloatTensor,
            ...
            "final_layer_norm": torch.FloatTensor,
            "embed_out": torch.FloatTensor,
        }
        """
        ...
        if "embed_in" in intervention_kwargs:
            if intervention_position is None:
                intervention_position = -1
            inputs_embeds[:, intervention_position, :] = intervention_kwargs["embed_in"]

        hidden_states = self.gpt_neox.emb_dropout(inputs_embeds)
        all_hidden_states["embed_in"] = hidden_states

        position_embeddings = self.gpt_neox.rotary_emb(hidden_states, position_ids)
        all_hidden_states["rotary_emb"] = position_embeddings

        presents = () if use_cache else None
        for i, (layer, layer_past) in enumerate(zip(self.gpt_neox.layers, past_key_values)):
            outputs = layer(
                hidden_states,
        ...
        if "final_layer_norm" in intervention_kwargs:
            if intervention_position is None:
                intervention_position = -1
            hidden_states[:, intervention_position, :] = intervention_kwargs["final_layer_norm"]
        all_hidden_states["final_layer_norm"] = hidden_states

        lm_logits = self.embed_out(hidden_states)

        # Apply intervention to logits if specified
        if "embed_out" in intervention_kwargs:
            if intervention_position is None:
                intervention_position = -1
            lm_logits[:, intervention_position, :] = intervention_kwargs["embed_out"]
        all_hidden_states["embed_out"] = lm_logits

        return IntervenableOutput(logits=lm_logits, hidden_states=all_hidden_states)
...
class IntervenableQwen3ForCausalLM(Qwen3ForCausalLM):
    """An intervenable version of Qwen2ForCausalLM.
    intervention_kwargs:
    {
        "model.embed_tokens": torch.FloatTensor,
